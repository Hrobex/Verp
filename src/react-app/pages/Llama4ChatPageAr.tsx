    ...
    <div className={`flex items-start gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`group relative max-w-2xl lg:max-w-3xl px-4 py-3 rounded-xl shadow-md ${isUser ? 'bg-blue-600' : 'bg-gray-700'}`}>
        ...
        <div className="absolute top-1 left-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => navigator.clipboard.writeText(message.content)} className="p-1.5 hover:bg-black/20 rounded-md" title="نسخ"><Copy size={14}/></button>
          {isUser && <button onClick={() => onEdit(message.id)} className="p-1.5 hover:bg-black/20 rounded-md" title="تعديل وإعادة الإرسال"><Edit2 size={14}/></button>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-2xl shadow-2xl">
        <h3 className="text-lg font-bold mb-4">تعديل الرسالة</h3>
        ...
        <div className="flex justify-end gap-4">
          <button onClick={onClose} className="px-4 py-2 bg-gray-600 rounded-md hover:bg-gray-500">إلغاء</button>
          <button onClick={() => onSave(message.id, editText)} className="px-4 py-2 bg-emerald-600 rounded-md hover:bg-emerald-500">حفظ وإعادة التوليد</button>
        </div>
      </div>
    </div>
  );
...
function Llama4ChatPageAr() {
  ...
  const handleSendMessage = useCallback(async (content: string, history: Message[]) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: { message: 'فشل تحليل استجابة الخطأ من الخادم.' }}));
        throw new Error(errorData.error?.message || response.statusText);
      }
      
      if (!response.body) throw new Error('جسم الاستجابة فارغ.');
      ...
      if (error.name !== 'AbortError') {
        setMessages(current => {
          const updated = [...current];
          updated[updated.length - 1].content = `**خطأ:** ${error.message}`;
          return updated;
        });
      }
  ...
  return (
    <>
      ...
      <meta name="description" content="ابدأ محادثتك مع مساعد الذكاء الاصطناعي Llama-4." />
      <meta name="robots" content="noindex, follow" />
      <link rel="canonical" href="https://aiconvert.online/ar/llama-4/chat" />
      ...
      <div className="flex flex-col bg-gray-900 text-white font-sans relative" style={{ minHeight: 'calc(100vh - 80px)' }}>
        <h1 className="sr-only">واجهة محادثة Llama-4</h1>
