// الملف: api/generate-prompt.ts (النسخة النهائية التي تتجنب خطأ Module)

// تم حذف السطر: import type { VercelRequest, VercelResponse } from '@vercel/node';
...
// ======================= التغيير الرئيسي هنا =======================
// غيّرنا تعريف الدالة لتقبل أي طلب (req: any) وأي رد (res: any)
// هذا يزيل الحاجة لاستيراد أي شيء من '@vercel/node' ويحل مشكلة التعارض
    ...
    try {
        ...
        for (const modelName of MODEL_FALLBACK_CHAIN) {
            try {
                ...
                if (!apiResponse.ok) {
                    throw new Error(responseData.error?.message || `Google API returned status ${apiResponse.status}`);
                }
        ...
        if (finalResultText) {
            ...
            console.error("All models in fallback chain failed. Last known error:", lastError);
