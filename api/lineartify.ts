// الملف: api/lineartify.ts
import type { VercelRequest, VercelResponse } from '@vercel/node';
import formidable from 'formidable';
import fs from 'fs';

// --- البيانات السرية والمنطق التشغيلي ---
const LINEARTIFY_API_URL = 'https://asartb-lld.hf.space/predict/';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (!imageFile || version === undefined) {
            return res.status(400).json({ error: 'Image file and version are required.' });
        }
        ...
        const apiResponse = await fetch(LINEARTIFY_API_URL, {
            method: 'POST',
        ...
        console.error("LineArtify backend error:", error.message);
